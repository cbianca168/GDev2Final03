...
public class QualitySelector : MonoBehaviour {
	...
	public struct Quality {
		...
		public HDAdditionalLightData clampedShadowResolutionTarget;
	}
	...
	void ApplyQuality(Quality quality, bool verbose) {
		...
		if(quality.clampedShadowResolutionTarget && quality.renderPipeline) {
			...
			if(hdrp) {
				...
				var atlasWidth = hdrp.currentPlatformRenderPipelineSettings.hdShadowInitParams.punctualLightShadowAtlas.shadowAtlasResolution; //UPGRADE_TODO: should be directional
				if(cascadeCount != 4) {
